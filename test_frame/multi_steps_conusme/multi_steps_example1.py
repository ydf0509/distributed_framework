...
@task_deco('queue_test_step1', qps=0.5, broker_kind=BrokerEnum.REDIS_LIST_AND_SET, concurrent_mode=ConcurrentModeEnum.GEVENT)
def step1(x):
    print(f'x 的值是 {x}')
    if x == 0:
        for i in range(1, 3):
...
@task_deco('queue_test_step2', qps=3, broker_kind=BrokerEnum.REDIS_LIST_AND_SET)
def step2(y):
    print(f'y 的值是 {y}')
    time.sleep(10)
